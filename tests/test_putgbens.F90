! Unit tests for putgbens() deprecated wrapper around putgbe().
! PUTGBENS simply delegates to PUTGBE with the same parameters.
program test_putgbens
  implicit none

  integer, parameter :: kf = 6

  ! Test 1: basic write with all valid data, no bitmap
  call run_case(.false., 'all_valid', (/ .true._1, .true._1, .true._1, .true._1, .true._1, .true._1 /), 1, .false.)

  ! Test 2: write with mixed bitmap (valid and invalid points)
  call run_case(.true.,  'mixed',    (/ .true._1, .false._1, .true._1, .false._1, .true._1, .true._1 /), 2, .false.)

  ! Test 3: write with ensemble parameters
  call run_case(.true.,  'ensemble', (/ .true._1, .true._1, .true._1, .true._1, .true._1, .true._1 /), 3, .true.)
...
contains
  ...
  subroutine run_case(use_bitmap, label, lb_in, stop_base, use_ensemble)
    ...
    write(filename, '("test_putgbens_",A,"_k",I0,".grb")') trim(label), kind(1.0)

    lugb = 31
    call baopenwt(lugb, trim(filename), iret_io)
    if (iret_io /= 0) stop (100 + stop_base)

    call putgbens(lugb, kf, kpds, kgds, kens, lb_in, f_in, iret)
  ...
  subroutine init_test_record(kpds, kgds, kens, f)
    ...
    ! Minimal, valid metadata for a tiny lat/lon grid (same as putgbe/putgben tests)
    kpds(1) = 7
    kpds(2) = 2
    kpds(3) = 255
    kpds(5) = 11
    ...
    ! Ensemble parameters (used only when explicitly set in run_case)
    kens = 0
