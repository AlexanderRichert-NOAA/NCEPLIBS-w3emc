...
program test_gbytec
  ...
  out(1) = char(0)
  call gbytec(out, in, iskip, nbits)
  ...
  out(1) = char(0)
  call gbytesc(out, in, iskip, nbits, nskip, n)
  if (ichar(out(1)) .ne. in(1)) stop 21
  ...
  do iter = 1, 2
    ...
    if (iter .eq. 1) then
      ...
      call gbytesc(out8, in2, iskip, nbits, nskip, n2)
    end if
    do i = 1, 8
       if (i .le. 2) then
  ...
  do iter = 1, 2
    ...
    if (iter .eq. 1) then
      ...
      call gbytesc(out5, in5, iskip, nbits, nskip, n5)
    end if
    do i = 1, 5
  ...
  do iter = 1, 2
    ...
    if (iter .eq. 1) then
      ...
      call gbytesc(out10, in5, iskip, nbits, 8, 5)
    end if
    do i = 1, 10
       ! print '(z2.2)', out10(i)
